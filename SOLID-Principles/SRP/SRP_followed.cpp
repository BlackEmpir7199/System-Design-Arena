...
// 2. SRP followed cart handler , does only 1 thing
class CartHandler
{
private:
...
// sepatare class for invoice generation
class InvoiceGenerator
{
public:
    InvoiceGenerator() {}
    void invoiceGenerate(int totalItems, int totalValue)
    {
...
// separate class for db functions
class DbHandler
{
public:
    DbHandler() {}
...
int main()
{
    ...
    int totalValue = cart->getTotalValue();

    gt.invoiceGenerate(totalItems, totalValue);
    db.saveToDB();
